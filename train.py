...
"""
A minimal training script for IRDM using PyTorch DDP.
"""
import torch
...
from utils.dataset_load import PT_Dataset
...
def main(args):
    ...
    # Note that parameter initialization is done within the IRDM constructor
    ...
    logger.info(f"IRDM Parameters: {sum(p.numel() for p in model.parameters()):,}")

    # Setup optimizer (we used default Adam betas=(0.9, 0.999) and a constant learning rate of 1e-4 in our paper):
    opt = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=0)

    # Setup data:
    dataset = PT_Dataset(args.data_path)
...
if __name__ == "__main__":
    # Default args here will train IRDM with the hyperparameters we used in our paper (except training iters).
    parser = argparse.ArgumentParser()
    parser.add_argument("--data-path", type=str, default="data/faceid.pt")
